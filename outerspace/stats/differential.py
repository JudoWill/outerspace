...
class SingleSampleDifferentialAbundance(BaseDifferentialAbundance):
    ...
    @staticmethod
    def calculate_differential_abundance_single(group1_df: pd.DataFrame, group2_df: pd.DataFrame) -> pd.DataFrame:
        """Calculate differential abundance scores between single samples

        Args:
            group1_df: DataFrame containing counts for first group
            group2_df: DataFrame containing counts for second group
        ...
        if group1_df.shape[1] != 1 or group2_df.shape[1] != 1:
            raise ValueError("Single-sample comparison requires exactly one sample per group")
        ...
        for umi in group1_df.index:
            ...
            lfc = BaseDifferentialAbundance._calculate_log2_fold_change(group1_count, group2_count)
    ...
    def run(self) -> pd.DataFrame:
        ...
        return self.calculate_differential_abundance_single(group1_df, group2_df)
...
class MannWhitneyDifferentialAbundance(BaseDifferentialAbundance):
    ...
    @staticmethod
    def calculate_differential_abundance(group1_df: pd.DataFrame, group2_df: pd.DataFrame) -> pd.DataFrame:
        """Calculate differential abundance scores using Mann-Whitney U test
    ...
    def run(self) -> pd.DataFrame:
        ...
        """
        group1_df, group2_df = self._prepare_dataframes()
        return self.calculate_differential_abundance(group1_df, group2_df)
...
class PairedTTestDifferentialAbundance(BaseDifferentialAbundance):
    ...
    @staticmethod
    def calculate_differential_abundance_paired(pre_df: pd.DataFrame, post_df: pd.DataFrame) -> pd.DataFrame:
        """Calculate differential abundance scores using paired t-test

        Args:
            pre_df: DataFrame containing pre-treatment counts
            post_df: DataFrame containing post-treatment counts
        ...
        for umi in pre_df.index:
            ...
            # Calculate log2 fold change
            lfc = BaseDifferentialAbundance._calculate_log2_fold_change(mean_pre, mean_post)
    ...
    def run(self) -> pd.DataFrame:
        ...
        """
        group1_df, group2_df = self._prepare_dataframes()
        return self.calculate_differential_abundance_paired(group1_df, group2_df)

...
def test_single_sample_differential_abundance():
    ...
    results = SingleSampleDifferentialAbundance.calculate_differential_abundance_single(group1_df, group2_df)
...
def test_single_sample_differential_abundance_identical():
    ...
    results = SingleSampleDifferentialAbundance.calculate_differential_abundance_single(df, df)
...
def test_single_sample_differential_abundance_empty():
    ...
    with pytest.raises(ValueError):
        SingleSampleDifferentialAbundance.calculate_differential_abundance_single(empty_df, df)
    with pytest.raises(ValueError):
        SingleSampleDifferentialAbundance.calculate_differential_abundance_single(df, empty_df)
...
def test_single_sample_differential_abundance_multiple_samples():
    ...
    with pytest.raises(ValueError):
        SingleSampleDifferentialAbundance.calculate_differential_abundance_single(df1, df2)
...
def test_single_sample_differential_abundance_with_allowed_list(partial_umi):
    ...
    result = SingleSampleDifferentialAbundance([partial_umi, partial_umi], ['group1', 'group2'], allowed_list=allowed_list).run()

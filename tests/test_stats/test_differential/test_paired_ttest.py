...
def test_paired_ttest_differential_abundance():
    ...
    results = PairedTTestDifferentialAbundance.calculate_differential_abundance_paired(pre_df, post_df)
    ...
    assert all(results['mean_post'] > results['mean_pre'])  # Post means should be higher
...
def test_paired_ttest_differential_abundance_identical():
    ...
    results = PairedTTestDifferentialAbundance.calculate_differential_abundance_paired(df, df)
...
def test_paired_ttest_differential_abundance_different():
    ...
    post_df = pd.DataFrame({
        'sample1': [30, 20, 10],  # Values are different but not consistently higher/lower
        'sample2': [35, 25, 15],
    }, index=[b"AAAAAA", b"TTTTTT", b"CCCCCC"])
    ...
    assert not all(results['effect_size'] > 0)  # Not all effect sizes should be positive
    assert not all(results['effect_size'] < 0)  # Not all effect sizes should be negative
...
def test_paired_ttest_differential_abundance_empty():
    ...
    with pytest.raises(ValueError):
        PairedTTestDifferentialAbundance.calculate_differential_abundance_paired(empty_df, df)
    with pytest.raises(ValueError):
        PairedTTestDifferentialAbundance.calculate_differential_abundance_paired(df, empty_df)
...
def test_paired_ttest_differential_abundance_mismatched():
    ...
    with pytest.raises(ValueError):
        PairedTTestDifferentialAbundance.calculate_differential_abundance_paired(pre_df, post_df)
...
def test_paired_ttest_differential_abundance_with_allowed_list(partial_umi):
    ...
    result = PairedTTestDifferentialAbundance([partial_umi, partial_umi], ['group1', 'group2'], allowed_list=allowed_list).run()

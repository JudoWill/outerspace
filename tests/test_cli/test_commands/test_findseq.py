...
def test_findseq_missing_reads():
    ...
    cli = Cli(args)
    with pytest.raises(ValueError, match="Please provide either -1 for single file or -1/-2 for paired files"):
        cli.run()
...
def test_findseq_with_example_data():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(os.path.join(temp_dir, 'shuffle.csv'), 'r') as f:
            ...
            expected_columns = ['read_id', 'UMI_5prime', 'protospacer', 'downstreamof_protospacer', 'UMI_3prime']
            assert all(col in header for col in expected_columns), f"Expected columns {expected_columns}, got {header}"
...
""")
        ...
        args = [
            ...
            config_file,
            '-1', fasta_file,
            '-o', os.path.join(temp_dir, 'output.csv')
        ]
        cli = Cli(args)
        cli.run()
